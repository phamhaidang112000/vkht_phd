...
import {ErrorMessageService} from 'src/app/services/core/error-message/error-message.service'
...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private checkStatus(ev: HttpResponseBase) {
    ...
    // const mess = `${this.i18n.fanyi('http.request.error')} ${ev.status}: ${ev.url}`;
    // this.messageService.add(mess);

    // this.notification.error(`${this.i18n.fanyi('http.request.error')} ${ev.status}: ${ev.url}`, errortext, {
    //   nzPauseOnHover: true,
    // });
  }
