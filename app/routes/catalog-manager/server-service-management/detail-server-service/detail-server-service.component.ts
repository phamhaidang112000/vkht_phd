...
@Component({
  selector: 'app-detail-server-service',
  templateUrl: './detail-server-service.component.html',
  styleUrls: ['./detail-server-service.component.less']
})
export class DetailServerServiceComponent implements OnInit , AfterViewInit{
  ...
  setBreadcrumb() {
    this.breadcrumbs = [
      ...
      {
        name: this.translateService.instant(!this.isEdit ? 'breadcrumb.server-service-management.add' : 'breadcrumb.server-service-management.edit'),
        route: '',
      },
  ...
  cancel() {
    this.router.navigate(["/catalog-management/server-service"]);
  }
  ...
  submitForm(checked: any) {
    ...
    if (this.addForm.valid) {
      ...
      this.serverServiceService.save(data).subscribe(res => {
        console.log(res);
        if (res) {
          if (!this.id) {
            if (res.data == "-2") {
              ...
              this.toastService.openSuccessToast(this.translateService.instant('catalog-management.server-service.add.success'));
              if (!this.continueAdd) {
                this.cancel();
              } else {
                this.addForm.reset();
            ...
            if (res.data != "1") {
              ...
              this.toastService.openSuccessToast(this.translateService.instant('catalog-management.server-service.update.success'));
              this.cancel();
            }
          }
        }
  ...
  loadData(id) {
    this.serverServiceService.detail(id).subscribe(res => {
      this.data = res.data;
