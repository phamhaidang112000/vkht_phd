...
@Component({
  ...
  styleUrls: ['./detail-storage.component.less']
})
export class DetailStorageComponent implements OnInit, AfterViewInit  {
  ...
  setBreadcrumb() {
    this.breadcrumbs = [
      ...
      {
        name: this.translateService.instant(!this.isEdit ? 'breadcrumb.storage-management.add' : 'breadcrumb.storage-management.edit'),
        route: '',
      },
  ...
  cancel() {
    this.router.navigate(["/catalog-management/storage"]);
  }
  ...
  submitForm(checked: any) {
    ...
    if (this.addForm.valid) {
      ...
      this.storageService.save(data).subscribe(res => {
        console.log(res);
        if (res) {
          if (!this.id) {
            if (res.data == "-2") {
              ...
              this.toastService.openSuccessToast(this.translateService.instant('catalog-management.storage.add.success'));
              if (!this.continueAdd) {
                this.cancel();
              } else {
                this.addForm.reset();
            ...
            if (res.data != "1") {
              ...
              this.toastService.openSuccessToast(this.translateService.instant('catalog-management.storage.update.success'));
              this.cancel();
            }
          }
        }
  ...
  loadData(id) {
    this.storageService.detail(id).subscribe(res => {
      this.data = res.data;
