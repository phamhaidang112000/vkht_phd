...
@Component({
  selector: 'app-detail-network',
  templateUrl: './detail-network.component.html',
  styleUrls: ['./detail-network.component.less']
})
export class DetailNetworkComponent implements OnInit, AfterViewInit {
  ...
  buildForm() {
    this.addForm = this.formBuilder.group({
      ...
      publicNetId: new FormControl(null, Validators.maxLength(500)),
      publicSubnetId: new FormControl(null, Validators.maxLength(500)),
    });
  }
  ...
  setBreadcrumb() {
    this.breadcrumbs = [
      ...
      {
        name: this.translateService.instant(!this.isEdit ? 'breadcrumb.network-management.add' : 'breadcrumb.network-management.edit'),
        route: '',
      },
  ...
  cancelConfirm() {
    if (this.data) {
      ...
      if (this.getValueField('name') || this.getValueField('publicNetId') || this.getValueField('publicSubnetId')) {
        this.isViewConfirmCancel = true;
      } else this.cancel();
    }
  }
  ...
  cancel() {
    this.router.navigate(["/catalog-management/network"]);
  }
  ...
  submitForm(checked: any) {
    ...
    if (this.addForm.valid) {
      ...
      let data = {
        id: this.id,
        name: fData.name,
        publicNetId: fData.publicNetId,
        publicSubnetId: fData.publicSubnetId
      }
      this.networkService.save(data).subscribe(res => {
        console.log(res);
        if (res) {
          if (!this.id) {
            // if (res.data == "-2") {
            ...
            // } else {
              this.toastService.openSuccessToast(this.translateService.instant('catalog-management.network.add.success'));
              if (!this.continueAdd) {
                this.cancel();
              } else {
                this.addForm.reset();
            ...
            if (res.data != "1") {
              this.toastService.openErrorToast(this.translateService.instant('catalog-management.network.delete.error', {
  ...
  loadData(id) {
    this.networkService.detail(id).subscribe(res => {
      this.data = res.data;
