...
@Component({
  ...
  styleUrls: ['./list-flavor.component.less']
})
export class ListFlavorComponent  implements OnInit, AfterViewInit, AfterViewChecked {
  searchForm: any;
  breadcrumbs: any = [];
  isBreadcrumb = false;
  total = 0;
  ...
  fetchSr() {
    ...
    this.flavorService.search(this.searchForm.value, this.request).subscribe(res => {
  ...
  onCreate() {
    this.router.navigate(['/catalog-management/flavor/add'])
  }
  ...
  goToEdit(data: any) {
    this.router.navigate(['/catalog-management/flavor/detail/' + data.id], { state: { page: this.request } });
    console.log(data.id)
  }
  ...
  callBackModalDelete(event: any) {
    this.flavorService.delete(this.selectedId).subscribe(res => {
      if (res.data != "1") {
